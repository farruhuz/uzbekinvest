const translationsUz = {
  home: "Asosiy",
  news: "Yangiliklar",
  services: "Xizmatlar",
  about: "Biz haqimizda",
  contact: "Bog’lanish",
  partners: "Hamkorlar",
  help: "Xujjatlar",
  login: "Kirish",
  ru: "Ru",
  uz: "Uz",
  contact__info:
    "Biz bilan bog'lanishinggiz uchun bizning tashkilotimizning turli sohalardagi manzillari",
  // BTNS words
  view__more__btn: "Batafsil",
  read__more__btn: "ko'proq o'qish",
  send: "Yuborish",
  // USEFULL links
  useFull__links: "Foydali havolalar",
  // Lasted Components
  last__news: "So'ngi Yangiliklar",
  // Tempus
  medical__assistance: "Tibbiy assistans",
  medical__assistance__info:"sayohat paytida tibbiy va transport yordamini tashkil etish bo'yicha xizmatlar to'plami. Turistga xizmatlar sug'urta polisi bo'yicha taqdim etiladi.",
  legal__assistance: "Yuridik assistans",
  legal__assistance__info:"fuqarolik, iqtisodiy, jinoiy, ma’muriy ish yuritishda ishtirok etmagan holda da’vo arizalari, apellyatsiya va kassatsiya shikoyatlarini, shuningdek boshqa protsessual hujjatlarni rasmiylashtirish;",
  technical__assistance: "Texnik assistans",
  technical__assistance__info:"Texnik komissarning xizmatlari (voqea joyiga jo'nab ketish, tekshirish, sug'urta tovonining miqdorini aniqlash, sug'urtalovchi (sug'urta qildiruvchi) uchun ekspert xulosasini tuzish, zararni qoplashda ishtirok etish).",
  personal__assistance: "Shaxsiy assistans",
  personal__assistance__info:"Mehmonxonalarda joylarni bron qilish, havo va temir yo'l transportiga chiptalar taqdim etish, tarjima xizmatlarini ko'rsatish, taksi xizmatlarini ko'rsatish, boshqa xizmatlar, dori vositalarini qidirish va etkazib berish.", 
  // Department Component
  depart: "Bo'limlar",
  depart__info__message:`«O'zbekinvest assistans» MCHJ tashkiloti bugungi kunda 4 bolimdan iborat va ular quyidagilar.`,
  depart__international: "Xalqaro hamkorlik bo’limi",
  depart__international__1: `«O‘zbekinvest Assistans» MCHJning Xalqaro hamkorlik bo‘limi jamiyatning xalqaro aloqalarini tashkillashtirish va uzviy davomiyligini ta’minlash uchun ma’suldir.`,
  depart__international__2: `Bo‘lim yangi hamkorlar bilan hamkorlikni o‘rnatish bilan bir qatorda, trevel polisi bilan O‘zbekiston Respublikasi hududiga kelgan sayyoxlarga kerakli assistans xizmatlarini tashkil etish uchun xam ma’suldir.`,
  depart__international__3: `O’zbekiston Respublikasida barcha viloyat markazlari hamda chekka hududlarida jami 127 tibbiy muassasalar bilan hamkorlik qilib kelmoqda. Bulardan eng yiriklari “SHOX MED CENTRE”, “EMU Clinic”, “MDS Servis”, “Doktor-D”, “Azimed Hospital” “Star med” kabi shifoxonalar shular jumlasidandir.`,
  depart__international__4: `Horijiy hamkorlarimiz Euroasia Assistance, Inter Assist, Missadena, Jiang Tai Global Assistance, VMA LLC, AP Companies Ltd, Global Voyadger Assistance, Remed Assistance, hamda Balt Assistance.`,
  depart__car: `Majburiy avto sug’urta turi bo’yicha da’vo ishlarini ko’rib chiqish bo’limi`,
  depart__car__1: `Kompaniya Call – markazi maʼlumotlariga asosan yo‘l transport hodisasi sodir bo‘lganda bevosita joyiga chiqish, sug‘urta hodisasining yuzaga kelishiga sabab bo‘lgan holatlarni o‘rganish, haydovchilar bilan birgalikda ixtiyoriy ravishda Habarnoma (halq tilida yevro-protokol) rasmiylashtirish, yo‘l transport hodisasida shikastlangan transport vositalarining holatini tekshirish yoki rasm (video)larga tushirish, sug‘urta hodisasi yuz berganligi to‘g‘risida sug‘urta qildiruvchi (haydovchi)lardan ariza olish, sug‘urta hodisasini o‘rganish, shu jumladan taqdim etilgan baxolash hisobotini xaqqoniyligini tekshirib, sug‘urta hodisasini tan olish yoki tan olmaslik to‘g‘risidagi komissiya xulosasini olish, sug‘urta tovonini to‘liq olish uchun zarur hujjatlarni to‘plashda aybdor va jabrlanuvchilarga yordam ko‘rsatish.`,
  depart__car__2: `Amalga oshirilgan ishlar bo‘yicha xar kunlik, xaftalik va oylik xisobotlar tayyorlash hamda jamiyat direktoriga taqdim etib borish.`,
  depart__may: `Ixtiyoriy sug'urta turlari bo'yicha da'volarni ko'rib chiqish bo'limi`,
  depart__may__1: `Kompaniya Call – markazi ma’lumotlariga asosan yo‘l-transport hodisasi sodir bo‘lganda bevosita joyiga chiqish, zarar ko‘rgan transport vositalarni va hujjatlarini videoga - suratga olish, zarur bo‘lganda evakuator xizmatlarini tashkil qilish, transport vositalariga texnik yordam ko‘rsatish, “hushyor haydovchi” va boshqa zaruriy xizmatlarini amalga oshirish.`,
  depart__may__2: `Ixtiyoriy sug‘urta turi bo‘yicha zarar ko‘rgan transport vositalarini voqea joyida boshlang‘ich baholash ishlarini to‘liq amalga oshirish.`,
  depart__may__3: `Mahalliy sug‘urta va assistans kompaniyalari murojaatiga ko‘ra transport vositalariga texnik yordam ishlarini amalga oshirish va tashkil qilish mumkin.`,
  depart__may__4: `Ixtiyoriy sug‘urta turi bo‘yicha, kelib tushayotgan murojaatlarni ko‘rib chiqib xulosa berish.`,
  depart__may__5: `Zarar ko‘rgan transport vositalarni ta’mirlash ishlari jarayonini bevosita avtoustaxonalarga chiqqan holda nazorat qilib, avtotrasport vositasi to‘liq ta’mirlangandan so‘ng jamiyat vakili, mijoz va avtoustaxonaning vakili ishtirokida zararlangan avtotransportni 3 (uch) tomonlama qabul qilish va topshirish dalolatnomasi rasmiylashtirilgan holda mijozga topshirishni ta’minlash.`,
  depart__may__6: `Amalga oshirilgan ishlar bo‘yicha har kunlik, haftalik va oylik hisobotlar tayorlash hamda jamiyat direktoriga taqdim etib borish.`,
  depart__may__7: `Bugungi kunda O’zbekiston Respublikasining barcha viloyatlari markazida va Toshektn shahrida yuqori sifatli texnik ustaxonalar bilan hamkorlik qilib kelinmoqda. Bular “JM-AVTO” MCHJ, “FORSAJ LYUKS AVTO” MCHJ, YATT SHAYUNUSOV B.R, “PIT STOP MOTORS” MCHJ (Avtoritet) MAJOR BUSINESS GROUP MCHJ (CARBOX) hamda AVTOTEXXIZMAT MCHJ kabi avto texnik ustaxonalar.`,
  depart__legal: "Yurudik Bo'lim",
  depart__legal__1: `Qonun talablariga asosan, Davlat mablag’larini qaytarish maqsadida sug’urta hodisasida aybdorligi tasdiqlangan shaxslardan regress huquqi asosida yetkazilgan moddiy zararlarni talab qilish va (yoki) sud orqali undirish.`,
  depart__legal__2: `“O‘zbekinvest Assistans” MChJ va uning mijozlarining manfaatlarini belgilangan tartibda iqtisodiy, fuqarolik va jinoyat ishlari bo‘yicha sudlarda, shuningdek, boshqa organlarda aybdorlarga murojaat qilish va “O‘zbekinvest Assistans” MChJ hamda uning mijozlari manfaatlarini himoya qiladi. `,
  depart__legal__3: `Da'vogar va sud tekshiruvlari natijalarini tahlil qiladi va umumlashtiradi, shuningdek shartnomalarning amalda yopilishi va bajarilishini tahlil qiladi.`,
  depart__legal__4: `Hujjatlarni huquqiy ekspertizadan o'tkazadi, shuningdek, "O'zbekinvest Assistance" MChJ va uning mijozlari tomonidan tuzilgan  shartnomalar va boshqa hujjatlarni tasdiqlash.`,
  // Est Component
  about__1: `“O‘zbekinvest Assistans” mas’uliyati cheklangan jamiyati ichki 
  bozordan tashqari xorijiy assistans kompaniyalarga ham xizmat ko‘rsatadi. 
  Bugungi kunda bizning hamkorlarimiz “Euroasia Assistance”, “Inter Assist”, 
  “Missadena”, “Jiang Tai Global Assistance”, “VMA” LLC, “AP Companies” Ltd, 
  “Global Voyadger Assistance”, “Remed Assistance” va “Balt Assistance” va boshqalar 
  kabi assistans kompaniyalari hisoblanadi.`,
  about__2: `2000 yilda tashkil etilgan “O‘zbekinvest Assistance” MCHJ O‘zbekiston Respublikasidagi asosiy assistans kompaniyalardan biri hisoblanadi.`,
  about__3: `Kompaniyaning asosiy afzalliklari`,
  //Footer component
  adress: "O‘zbekiston Respublikasi, Toshkent, 100017, ko‘ch. Chupon ota, 6.",
  // Kontakt page
  direct__number: "To'g'ridan-to'g'ri aloqa raqami",
  short__number: "Qisqa raqam",
  adress__map: "Manzil",
  phone__map: "Теlefon",
  follow_us__map: "Obuna bo'ling",
  email__map: "Elektron pochta",
  //  ABOUT Page
  founders: "Biz haqimizda",
  founders__1: `2000 yilda tashkil etilgan “O‘zbekinvest Assistance” MCHJ O‘zbekiston Respublikasidagi asosiy assistans kompaniyalardan biri hisoblanadi.`,
  founders__2: `“O‘zbekinvest Assistans” mas’uliyati cheklangan jamiyati ichki bozordan tashqari xorijiy assistans kompaniyalarga ham xizmat ko‘rsatadi. Bugungi kunda bizning hamkorlarimiz “Euroasia Assistance”, “Inter Assist”, “Missadena”, “Jiang Tai Global Assistance”, “VMA” LLC, “AP Companies” Ltd, “Global Voyadger Assistance”, “Remed Assistance” va “Balt Assistance” va boshqalar kabi assistans kompaniyalari hisoblanadi.`,
  founders__3: "Rahbariyat",
  founders__4: "Hamkorlar",
  // PARTNERS Page
  partners__title__1: `• Assistans xizmatlari - texnik, tibbiy va yuridik yordam ko'rsatish bo'yicha 21 yildan ortiq tajriba.`,
  partners__title__2: `• Noyob ma'lumotlar bazasi, telefon suhbatlarini yozib olish bilan integratsiyalashgan IP-telefoniya bilan jihozlangan kechayu kunduz ishlaydigan dispetcherlik markazi.`,
  partners__title__3: `• Ish kechayu-kunduz, tanaffussiz, dam olish va bayramlarsiz amalga oshiriladi.`,
  partners__title__4: `• Kasbiy tayyorgarlikka ega kadrlar, jumladan, yuqori malakali shifokorlar va huquqshunoslar.`,
  partners__title__5: `• Kasalxonaga yotqizishni tashkil etish.`,
  partners__title__6: `• Repatriatsiyani tashkil etish.`,
  // Boxses OTDEL 1
  otdel__1__title__1: "Xorijiy hamkorlarimiz",
  otdel__1__subtitle__1: `Euroasia Assistance, Inter Assist, Missadena, Jiang Tai Global
    Assistance, VMA LLC, AP Companies Ltd, Global Voyager
    Assistance, Remed Assistance и Balt Assistance.`,
  otdel__1__title__2: "Mahalliy Tibbiy va texnik hamkorlarimiz",
  otdel__1__subtitle__2: `«“SHOX MED CENTRE”, “EMU Clinic”, “MDS Servis”, “Doktor-D”,
    “Azimed Hospital” “Star med” ООО "MAK MOTORS", ООО " JM-AVTO
    MCHJ", "Avtoritet", «Forsaj» и др;`,
  otdel__1__title__3: "Sugurta kompaniyalar orasidagi hamkorlarimiz",
  otdel__1__subtitle__3: `“O‘zbekinvest EISK” AJ, “ARIA” sug‘urta tashkiloti, “Ingo Uzbekistan” sug‘urta kompaniyasi.`,
  // Boxses OTDEL 2 Majburiy
  otdel__2__title__1: "ОСГОВТС",
  otdel__2__subtitle__1: `transport vositalari yegalarining fuqarolik javobgarligini majburiy sug‘urta qilish’ siyosatini anglatadi. Ushbu turdagi avtoulov sug‘urtasi avtomobilning har bir yegasi uchun zarurdir va tegishli hujjatlarning yo‘qligi jarimaga olib kelishi mumkin. 
    OSGO VTS siyosati faqat uchinchi shaxslarning mol-mulkiga yoki sog‘lig‘iga yetkazilgan zararni qoplashiga ye’tibor qaratish lozim. OSAGO sug‘urtasi yegasining transport vositalariga yoki avtohalokat natijasida olingan jarohatlarga yetkazilgan zarar uchun kompensatsiya bermaydi. Voqea sodir bo‘lgan taqdirda, fuqarolik javobgarligini sug‘urta qilish polisi buzilgan mol-mulk yegalari tomonidan moddiy da’volardan himoya qiladi, shuningdek, hodisa paytida zarar ko‘rgan odamlarning sog‘lig‘iga zarar yetkazadi.
    Agar transport vositasini boshqarish uchun bir nechta shaxs yo‘l qo‘yilgan bo‘lsa, OSAGO sug‘urta shartnomasi har bir haydovchi uchun alohida-alohida berilishi kerak. OSGOVTS siyosatida sug‘urta tariflari barcha sug‘urtalovchilarga qonun hujjatlarida belgilangan yagona qoidalar bo‘yicha qo‘llaniladi. Ushbu sug‘urta turi bo‘yicha sug‘urta summasi 40 million so‘m miqdorida belgilanadi. Shu bilan birga, sug‘urta summasini ajratish qonun bilan quyidagi chegaralar doirasida belgilanadi: hayot va sog‘liqqa yetkazilgan zararni qoplash uchun – 65%; jabrlanuvchilarning mol – mulkiga yetkazilgan zararni qoplash uchun-35%.
    `,
  otdel__2__subtitle__1__2:"transport vositalari yegalarining fuqarolik javobgarligini majburiy sug‘urta qilish’ siyosatini anglatadi.",
  otdel__2__title__2: "YEVROPROTOKOL",
  otdel__2__subtitle__2: `Yo‘l-transport hodisasi sodir bo‘lganligi to‘g‘risida ichki ishlar 
    organi xodimlari ishtirokisiz sug‘urtalovchini avtohalokatga uchragan haydovchilar tomonidan 
    bildirishnoma tuzish orqali xabardor qilish.2021-yil 25-noyabrdan O‘zbekistonda Yevropa protokoli 
    ish boshladi.Bu shuni anglatadiki, tomonlarning o'zaro kelishuvi bilan inspektorlar ishtirokisiz
    baxtsiz hodisani rasmiylashtirish mumkin bo'ladi.GAI. Asosiysi, har ikki tomonning fuqarolik 
    javobgarligi sug'urtalangan bo'lishi kerak.Sug'urta polisini sotib olgandan keyin beriladigan 
    va sug'urta kompaniyasining veb-saytidan yuklab olish mumkin bo'lgan maxsus shakllarni to'ldirgandan so'ng, 
    baxtsiz hodisaning ikkala ishtirokchisi ularni o'z sug'urta kompaniyasiga olib boradi. Shundan so'ng, 
    sug'urtachilar tovon to'lash bo'yicha muzokaralar olib borish uchun ishga kelishadi.`,
  otdel__2__subtitle__2:`Yo‘l-transport hodisasi sodir bo‘lganligi to‘g‘risida ichki ishlar 
    organi xodimlari ishtirokisiz`,
  otdel__2__subtitle__2__2:`Sug'urtalovchini ichki ishlar xodimlari ishtirokisiz yo'l-transport hodisasi sodir bo'lganligi to'g'risida xabardor qilish `,

  medical__assistance__info__1: "Tibbiy yordam - Tibbiy yordam - sayohat paytida tibbiy va transport yordamini tashkil etish bo'yicha xizmatlar to'plami. Turistga xizmatlar sug'urta polisi bo'yicha taqdim etiladi.",  
  medical__assistance__info__2: "Ba'zida sug'urta yordami va sug'urta xizmati kabi terminologiya qo'llaniladi.", 
  medical__assistance__info__3: "Ushbu faoliyat bilan shug'ullanuvchi xizmat ko'rsatuvchi kompaniya tibbiy muassasani topish, kerakli mutaxassisning qabuliga yozilish, xizmatlar uchun haq to'lash va sug'urtalanganni doimiy yashash mamlakatiga olib borish bilan shug'ullanadi.", 
  yurudik__assistance__info__1:"huquqiy yordam bo'limi:",
  yurudik__assistance__info__2:"og'zaki va yozma yuridik maslahatlar berish;",
  yurudik__assistance__info__3:"fuqarolik, iqtisodiy, jinoiy, ma’muriy ish yuritishda ishtirok etmagan holda da’vo arizalari, apellyatsiya va kassatsiya shikoyatlarini, shuningdek boshqa protsessual hujjatlarni rasmiylashtirish;",
  yurudik__assistance__info__4:"surishtiruv organlarida va sudda ma'muriy ish yuritish jarayonida manfaatlarni ifodalash. Ma'muriy javobgarlik to'g'risidagi qarorlar ustidan shikoyat qilish;",
  yurudik__assistance__info__5:"baxtsiz hodisa natijasida yetkazilgan moddiy va ma’naviy zararni qoplash to‘g‘risidagi da’volar bo‘yicha har qanday instansiya sudida fuqarolik, xo‘jalik protsessida manfaatlarni ifodalash;",
  yurudik__assistance__info__6:"baxtsiz hodisa natijasida etkazilgan moddiy va ma'naviy zararni qoplash to'g'risidagi fuqarolik da'volari bo'yicha har qanday instansiya sudida jinoyat protsessida manfaatlarni ifodalash;",
  yurudik__assistance__info__7:"sug'urta tovonini to'lash to'g'risidagi da'volar bo'yicha har qanday instansiya sudida fuqarolik, xo'jalik protsessida manfaatlarni ifodalash;",
  yurudik__assistance__info__8:"moddiy zararni minimallashtirish maqsadida avtotovar tadqiqotlari/ekspertizalarini huquqiy tekshirish.",
  techno__assitance__info__1:"Texnik komissarning xizmatlari (voqea joyiga jo'nab ketish, tekshirish, sug'urta tovonining miqdorini aniqlash, sug'urtalovchi (sug'urta qildiruvchi) uchun ekspert xulosasini tuzish, zararni qoplashda ishtirok etish).",
  techno__assitance__info__2:"Evakuator xizmatlari: yo‘l-transport hodisasida texnik nosozlik yoki shikastlanganda avtomobilni texnik xizmat ko‘rsatish shoxobchasiga tashish (O‘zbekiston Respublikasining butun hududi bo‘ylab).",
  techno__assitance__info__3:"- O'zbekiston Respublikasining istalgan mintaqasidagi xizmat ko'rsatish shoxobchalarida, polis berilgan joydan qat'i nazar, ta'mirlash ishlarini qamrab olish.",
  techno__assitance__info__4:"- sug'urta kompaniyasining sug'urtalangan shaxs oldidagi kafolat majburiyatlari.",
  personal__assistance__info__1:"Mehmonxonalarda joylarni bron qilish, havo va temir yo'l transportiga chiptalar taqdim etish, tarjima xizmatlarini ko'rsatish, taksi xizmatlarini ko'rsatish, boshqa xizmatlar, dori vositalarini qidirish va etkazib berish.",

  // partners PArtners
  parners__four__1:"Biz ARIA jamoasi, sizni jamiyatimizning bir qismi bo'lishingizni xohlaymiz. Bizning jamoamiz xotirjamlik va isonchni taqdim etadi. Bu esa mijozlarga biznesni yanada rivojlantirish masalalariga e’tibor qaratishga imkon yaratadi.",
  parners__four__2:"“Avtoritet Group” kompaniyalar guruhining hamkorlari o'z faoliyatlarini quyidagi yo'nalishlarda olib boradilar: avtomobillarni sotish, transport vositalariga kafolatli va texnik xizmat ko'rsatish, yuk tashish, ehtiyot qismlarni etkazib berish",
  parners__four__3:"“O‘zbekinvest” eksport-import aksiyadorlik jamiyati sug'urta kompaniyasi umumiy sug'urta bozorida o‘z faoliyatini olib boradi ва sug'urta faoliyati klassifikatoriga ko‘ra mavjud 17 turdagi sug‘urtaning barchasi билан shug‘ullanadi",
  parners__four__4:`INGO-UZBEKISTAN kompaniyasi – biznes manfaatlarining kompleks himoya dasturlari hamda xususiy shaxslar uchun sug‘urta mahsulotlarinnig keng qamrovini o‘z ichiga olgan universal xizmat portfeliga ega O‘zbekistonnning eng yirik sug‘urtachilaridan biridir.`,
  // Horihiy hamkorlarimiz
  friend__1__1__title:"Allianz Global Assistance (sobiq Mondial Assistance)",
  friend__1__1__subtitle:"Allianz o'z tarixini 1890 yilga borib taqaladi. U Myunxenda tashkil etilgan, asoschisi Myunxen qayta sug'urta jamiyati rahbari - Karl fon Thieme, u ham Europaische Reiseversicherung Aktiengesellschaft (ERV) asoschisi edi.",
  friend__1__1__subtitle__2:"Mondial Assistance 2014 yilda Allianz Global Assistance nomini oldi.",
  friend__1__2__title:"CORIS Group",
  friend__1__2__subtitle:"Kompaniya 1987 yilda Frantsiyada, Parijda tashkil etilgan. Bugungi kunda CORIS International xolding kompaniyasi dunyodagi eng yirik yordam tarmoqlaridan biri hisoblanadi. U sug'urtalangan shaxslarga yordam ko'rsatishni tashkil etish, turli turdagi sug'urta yo'qotishlarini qoplashga ixtisoslashgan.",
  friend__1__3__title:"Global Voyager Assistance ",
  friend__1__3__subtitle:"Xolding o'z faoliyatini 1998 yilda boshlagan. 2000 yilda kompaniya xizmat ko'rsatuvchi kompaniyalar xalqaro assotsiatsiyasi - Xalqaro yordam guruhiga (IAG) a'zo bo'ldi va bu dunyoning etakchi yordamchi kompaniyalari bilan hamkorlik qilish imkonini berdi.",
  // Horihiy hamkorlarimiz 2
  friends__1__1__title:"Shox International Hospital",
  friends__1__1__subtitle:`"Shox International Hospital" -yuqori aniqlikdagi diagnostika, ilmiy-texnik laboratoriya, jarrohlik operatsiyalari va boshqa turdagi tibbiy xizmat ko'rsatishga ixtisoslashgan O'zbekistondagi klinikalarning eng yirik xususiy tibbiyot tarmog'i.`,
  friends__1__1__subtitle__2:`Xalqaro sifat darajasi, alohida xizmat va yuqori texnologiyali innovatsion echimlar – bizning klinika faoliyatining asosiy ustuvor yo'nalishlari. Ilg'or xalqaro amaliyot va standartlarni joriy etish "Shox International Hospital" ga jahon miqyosidagi eng yaxshi klinikalar darajasiga erishish imkonini berib, o'zbek tabobati xalqaro standartlarga mos kelishi mumkinligini isbotladi.`,
  friends__1__1__subtitle__3:`"Shox International Hospital" tibbiyot markazlari tarmog'iga 3 tasi Toshkent shahrida, 2 tasi O'zbekiston Respublikasi hududlarida joylashgan 5 ta amaliy tibbiyot klinikalari kiradi.`,
  friends__1__1__subtitle__4:`- Toshkent shahri, Yakkasaroy tumani, kichik xalqa yo'li ko'chasi, 70-a uy.`,
  friends__1__1__subtitle__5:`- Toshkent shahri, Mirobod tumani, Oybek ko'chasi, 34-uy.`,
  friends__1__1__subtitle__6:`- Toshkent shahri, Uchtepa tumani, M. Shayxzoda ko'chasi, 31A uy.`,
  friends__2__1__title:"Стар Мед Центр",
  friends__2__1__subtitle:`Star Med Center - bu ko'p tarmoqli klinika bo'lib, u erda sog'ligingiz bilan bog'liq har qanday masala bo'yicha murojaat qilishingiz mumkin. Bizda turli kasalliklarni davolash va tashxislash bo'yicha ko'p yillik tajribaga ega eng yaxshi mutaxassislar ishlaydi`,
  friends__2__1__subtitle__2:`Barcha qulayliklarga ega stasionar`,
  friends__2__1__subtitle__3:`Jixozlangan operasion xonalar`,
  friends__2__1__subtitle__4:`Tashxis va maslahatlar`,
  friends__2__1__subtitle__5:`Zamonaviy texnologiyalar `,
  friends__2__1__subtitle__6:`Samarali fizioterapiya`,
  friends__2__1__subtitle__7:`Malakali xodimlar`,
  // Horihiy hamkorlarimiz 3
  friendss__1__1__title:"Oʼzbekinvest",
  friendss__1__1__subtitle:`«Oʼzbekinvest» eksport-import aksiyadorlik jamiyati sugʼurta kompaniyasi umumiy sugʼurta bozorida oʼz faoliyatini olib boradi va sugʼurta faoliyati klassifikatoriga koʼra mavjud 17 turdagi sugʼurtaning barchasi bilan shugʼullanadi`,
  friendss__1__1__subtitle__2:`Tashkil etilgan sana:`,
  friendss__1__1__subtitle__3:`1994 yil aprel – «Oʼzbekinvest» sugʼurta kompaniyasining tashkil etilishi (Oʼzbekiston Respublikasi Vazirlar Mahkamasining 1994 yil 13 apreldagi 206 sonli Qarori)`,
  friendss__1__1__subtitle__4:`1997 yil fevral – Eksport va investitsiyalarni sugʼurtalash boʼyicha mamlakatimizning rasmiy agenti mavqeyi berilib, «Oʼzbekinvest» eksport-import milliy sugʼurta kompaniyasi etib qayta tashkil etildi (Oʼzbekiston Respublikasi Prezidentining 1997 yil 18 fevralьdagi PQ-1710 sonli hamda Oʼzbekiston Respublikasi Vazirlar Mahkamasining 1997 yil 28 fevralьdagi 113 sonli Qarori)`,
  friendss__1__1__subtitle__5:`2020 yil 1 iyulda Oʼzbekiston Respublikasi Prezidentining "Eksport faoliyatini moliyalashtirish va sugʼurta mexanizmlarini kengaytirish toʼgʼrisida" gi farmoni ijrosi doirasida va Respublika Vazirlar Mahkamasining tayinlanishiga javoban Oʼzbekiston, kompaniya aktsiyadorlik jamiyatiga aylantirildi.`,
  friendss__1__1__subtitle__6:`Kompaniya Ustav jamgʼarmasi: 282,2 mlrd soʼm.`,
  friendss__1__1__subtitle__7:`Kompaniya taʼsischilari: Oʼzbekiston Respublikasi Investitsiyalar va tashqi savdo vazirligi (83,33%)`,
  friendss__1__1__subtitle__8:`Kompaniya missiyasi – Maqbul narxlardagi yuqori sifatli sugʼurta xizmatlarini taqdim etish orqali Oʼzbekiston Respublikasi sugʼurta bozorining rivojlanishiga koʼmaklashish.`,
  // Avto sugurta
  avto__s__1__title:`TVEFJMS`,
  avto__s__1__subtitle:`Avtotransport vositalari egalarining uchinchi shaxslarning javobgarligini majburiy sug‘urtasi (TVEFJMS) – yo‘l-transport hodisasi natijasida jabrlangan shaxslarga tovon to‘lashga qaratilgan qonun hujjatlarida belgilangan sug‘urta turi.`,
  avto__s__1__subtitle__2:`Agar baxtsiz hodisa sug'urtalangan shaxsning aybi bilan sodir bo'lsa, aksariyat hollarda bunday sug'urta shartnomasi unga o'z byudjetiga jiddiy zarar etkazmasdan jabrlanganlarga tovon to'lash imkonini beradi. Shunday qilib, avtomobil sug'urtasining ushbu turi uning barcha ishtirokchilari uchun baxtsiz hodisa oqibatlarini engillashtiradi va ko'pincha ushbu noxush hodisaning barcha oqibatlarini butunlay yumshata oladi. OSAGO siyosati bir yilgacha bo'lgan muddatga chiqariladi, undan keyin uni uzaytirish talab etiladi. Shu bilan birga, shartnomani avtomobil egasi ham, uni boshqarishga tan olingan boshqa shaxslar ham tuzishlari mumkin.`,
  avto__s__1__subtitle__3:`Qonunda OSAGO siyosati bo'yicha kompensatsiya olinmaydigan bir qator holatlar nazarda tutilgan, masalan, agar voqea sodir bo'lgan shaxs alkogol yoki giyohvandlik holatida bo'lsa yoki yo'l harakati qoidalarini buzgan holda avtomobilni boshqargan bo'lsa. O‘zbekistonda OSAGO siyosati bo‘yicha to‘lovlar muayyan baxtsiz hodisani sug‘urta hodisasi deb e’tirof etish to‘g‘risidagi akt imzolangandan keyin besh kun ichida amalga oshiriladi. Bunday holda, quyidagi to'lov tartibi qo'llaniladi:`,
  avto__s__1__subtitle__4:`Jabrlanuvchining mol-mulkiga zarar yetkazilganda tovon to'lash sug'urta summasining 35 foizidan oshmasligi kerak`,
  avto__s__2__title:`Yo’l  transport hodisasi tog’risidagi xabarnoma`,
  avto__s__2__subtitle:`“Oʼzbekiston Respublikasining ayrim qonun hujjatlariga oʼzgartish va qoʼshimchalar kiritish toʼgʼrisida”gi Qonuni bilan kiritilgan oʼzgarishlarga koʼra, yoʼl-transport hodisasi toʼgʼrisidagi xabarnoma (Evroprotokol)– sodir etilgan yoʼl-transport hodisasi haqida sugʼurtalovchiga xabar berish uchun hodisa sodir etilgan joyda unda ishtirok etgan haydovchilar tomonidan ixtiyoriy ravishda toʼldiriladigan, transport vositalari egalarining fuqarolik javobgarligi vujudga kelganligi faktini tasdiqlovchi hujjat hisoblanadi.`,
  avto__s__2__subtitle__2:"2021-yil 5-noyabrdan O‘zbekistonda Yevropa protokoli ish boshlagan",
  avto__s__2__subtitle__3:`Shuningdek, «Evroprotokol» joriy etilishi bilan yoʼl harakati xavfsizligi xodimlari va haydovchilar oʼrtasidagi bevosita muloqotni amalga oshirishda “inson omili”ga va ortiqcha qogʼozbozlikka va fuqarolarni ortiqcha sarson boʼlishiga chek qoʼyishga, shu bilan birga har bir sodir etilgan yoʼl-transport hodisasini rasmiylashtirish uchun yoʼl-patrulь xizmati inspektorlarining oʼrtacha 2-3 soat vaqti tejalishi orqali tan jarohati bilan bogʼliq yoʼl transport hodisasini koʼrib chiqishda sifat darajasini oshirish hamda avtomobil yoʼllarida tirbandliklarning oldini olishda samara beradi.`,
  avto__s__2__subtitle__4:`Eng katta oʼzgarishlardan biri bu — haydovchilar tomonidan xabarnomani rasmiylashtirish maʼmuriy javobgarlikka tortilishni istisno etuvchi holat etib belgilanligi hisoblanadi. Endilikda xabarnomani rasmiylashtirgan haydovchilar maʼmuriy javobgarlikka tortilmaydi.`,
  avto__s__2__subtitle__5:"Tomonlarning o'zaro kelishuvi bilan inspektorlar ishtirokisiz baxtsiz hodisani rasmiylashtirish mumkin bo'ladi.",
  // Rahbariyat
  direktor__name__1:"Maxmudov Avzal Mamadjanovich",
  direktor__name__2:"Yakubov Toxir Yusufjanovich",
  direktor__name__3:"Axmetov Kabul Ataevich",
  direktor__position__1:"Direktor",
  direktor__position__2:"Direktor o'rinbosari",
  direktor__position__3:`“O’zbekinvest Assistans” MCHJ Bosh menejeri`,
  direktor__phone:"Telefon: ",
  direktor__reception__day:"Qabul kunlari: ",
  direktor__reception__day__1:"Dushanba 8:00-11:00, Chorshanba 8:00-11:00, Juma 8:00-11:00",
  direktor__reception__day__2:"Seshanba 8:00-11:00, Payshanba 8:00-11:00, Shanba 8:00-11:00",
  direktor__reception__day__3:"Dushanba 8:00-11:00, Chorshanba 8:00-11:00, Juma 8:00-11:00",
  direktor__email:"Elektron manzil: ",
  direktor__content__1__1:`
  Maxmudov Avzal Mamajanovich 1988 yil 6 avgust kuni Toshkent shahrida tug’ilgan. 
  Ma’lumoti oliy. 2009 yil Toshkent davlat iqtisodiyot universiteti bakalavr darajasi 
  va 2011 yil Toshkent davlat iqtisodiyot universiteti magistraturani ta’momlagan.`,
  direktor__content__1__2:`2021 yil 6 sentyabrdan “O’zbekinvest Assistans” MCHJ Direktori`,

  direktor__content__2__1:`
  Yakubov Toxir Yusufjanovich 1989 yil 22 oktyabr kuni Toshkent shahrida tug’ilgan. 
  Ma’lumoti magistr. 2013 yil Toshkent davlat iqtisodiyot universiteti bakalavr va 
  2015 yil Toshkent davlat iqtisodiyot universiteti magistr unvonini olgan.`,
  direktor__content__2__2:`2021 yil 6 sentyabrdan “O’zbekinvest Assistans” MCHJ Direktor o’rinbosari`,

  direktor__content__3__1:`
  Axmetov Kabul Ataevich 1964 yil 18 avgust kuni Toshkent shahrida tug’ilgan. 
  Ma’lumoti oliy. 1986 yil Toshkent davlat universiteti bakalavr va 1995 yil 
  Toshkent davlat iqtisodiyot universiteti magistr unvonini olgan.`,
  direktor__content__3__2:`2021 yil 6 sentyabrdan “O’zbekinvest Assistans” MCHJ Ixtiyoriy sug’urta turlari bo’yicha da’vo ishlarini ko’rib chiqish bo’limi bosh menejeri`,


};

export default translationsUz;
